...
def send_tx(w3: Web3, tx: dict, pk: str, description: str) -> bool:
...
def main() -> None:
    ...
    usdc_e_raw = usdc_e_contract.functions.balanceOf(wallet).call()
    pusd_raw = pusd_contract.functions.balanceOf(wallet).call()
    print(f"  USDC.e balance: ${usdc_e_raw / 1e6:.6f}")
    print(f"  pUSD   balance: ${pusd_raw / 1e6:.6f}\n")
    ...
    print(f"--- Step 2: Wrap ${usdc_e_raw / 1e6:.6f} USDC.e → pUSD ---")
    # Use 'pending' nonce to account for any still-pending transactions
    ...
    usdc_e_after = usdc_e_contract.functions.balanceOf(wallet).call() / 1e6
    pusd_after = pusd_contract.functions.balanceOf(wallet).call() / 1e6
    ...
    if pusd_after > 0:
        ...
        print("\npUSD balance still 0 — check RPC state or wait a block and recheck.")
